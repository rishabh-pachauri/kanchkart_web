...
export const metadata = {
  title: "Bulk Shipping Labels & QR Generator | KanchKart Admin"
};
...
export default async function AdminBulkShippingLabelsPage({
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-6 md:p-12 text-slate-950 font-sans print:p-0 print:bg-white space-y-12">
      {/* Top Action Bar (Hidden when printing) */}
      <div className="mx-auto max-w-4xl flex items-center justify-between print:hidden">
        <Link href="/admin/orders" className="text-sm font-semibold text-slate-600 hover:text-slate-900">
          ← Back to Admin Orders Pipeline
        </Link>
        <div className="flex items-center gap-4">
          <span className="text-xs font-bold bg-slate-200 px-3 py-1.5 rounded-lg text-slate-700">
            {orders.length} Batch Labels Ready
          </span>
          <PrintLabelButton />
        </div>
      </div>
      ...
      <div className="mx-auto max-w-4xl space-y-12 print:space-y-0 print:max-w-none">
        {orders.map((order) => {
          ...
          return (
            <div
              key={order.id}
              className="rounded-2xl border-2 border-slate-900 bg-white p-8 shadow-2xl print:max-w-none print:rounded-none print:border-2 print:border-black print:p-6 print:shadow-none print:break-after-page"
            >
              {/* Label Header */}
